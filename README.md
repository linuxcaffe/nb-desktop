...
`NB_WEB_URL`) and a local API token, auto-generated by nb-web itself at
`~/dev/nb-web/.api_token` on first start (override the path this script
reads with `NB_WEB_API_TOKEN_FILE`). Unlike Import to nb, this script is
*not* standalone — it's a thin client over nb-web's own item-creation logic
